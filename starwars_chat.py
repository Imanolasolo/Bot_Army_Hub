...
def main():
    ...
    st.set_page_config(page_title="Star Wars AI", page_icon=":starwars:")
    ...
    with col1:
        st.image('starwars_logo.png', width=100)
    with col2:
        st.header("Star Wars - FAQ :robot_face:")

    with st.sidebar:
        
        st.subheader(":file_folder: Star Wars FAQ")
        
        #use_sample_pdf = st.checkbox("Talk with BOTARMY_HUB Bot")
        #if use_sample_pdf:
        sample_pdf_path = os.path.join(os.getcwd(), "starwars_pdf.pdf")
        ...
        train = st.button("Talk with Star Wars AI")
        if train:
        ...
        with st.expander("Expand questions"):
            ...
            instrucciones = [
                "1. What is Star Wars?.",
                "2. Who is Luke Skywalker?",
                "3. Give me a brief explanation about Han solo.",
                "4. Where is Tatooine?"
            ]
